...
- 分享链接解析覆盖 `vless://`、`vmess://`、`trojan://`、`ss://`、`socks://`、
  `http://`、`https://`、`wireguard://`、`anytls://`、`hysteria2://` 和 `hy2://`。
...
- 基于 ScanKit 的二维码能力：摄像头/相册扫码导入分享链接，节点详情与导出页生成二维码分享。
- 提供诊断日志面板、Native 运行状态和流量统计（含累计流量）展示。
...
| 扫码与分享 | 可用 | 文本与二维码导入导出均可用：ScanKit 摄像头/相册扫码导入，节点详情与导出页生成二维码 |
