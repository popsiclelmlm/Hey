...
| Xray 配置生成 | 🟡 81% | 普通节点生成 TUN/metrics/DNS/routing/HTTP 代理和本地 SOCKS 配置，HTTP/SOCKS 代理支持局域网共享监听，SOCKS 支持启动前动态端口；完整自定义 Xray config 可校验后原样运行；代理链和策略组 JSON 可生成多跳/负载均衡 outbounds，且添加节点页可从已有普通节点创建代理链/策略组；订阅正则动态成员与路由规则高级出站目标 UI 仍待补 |
