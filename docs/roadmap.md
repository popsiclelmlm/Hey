...
| 订阅管理 | 🟡 96% | 多分组 + 旧版迁移 + 编辑/重排/批量更新全部 + 订阅级不安全 URL 开关 + 当前/All 可见范围批量删除/去重/测速失败清理 + 自动更新设置/前台到期刷新 + 本地 HTTP 代理经由更新 + WorkScheduler 后台调度接线 + 订阅链接二维码/系统分享；订阅 URL 剪贴板粘贴提示已接入 i18n；订阅抓取支持 v2rayNG 风格 URL 内嵌 `user:pass@host` Basic Auth；待真机触发回归 |
