...
| Persistent app settings | Present for core VPN, DNS, DNS hosts, IPv6, mux/XUDP/fragment, sniffing, log, speed display, UI mode, all-groups display, double-column display, real-ping concurrency, delete confirmation, immediate scan startup, and routing strategy; selected values are applied at app launch/foreground, save time, connection start, batch test time, Scanner startup, destructive actions, or node list rendering, including Harmony color mode, all-groups node aggregation, double-column node list lanes, VPN interface DNS, generated Xray `dns.hosts`, IPv6 routing, outbound Happy Eyeballs, mux settings, outbound fragment settings, speed metrics generation/display, delay-test concurrency, scan auto-launch, and deletion confirmation. |
