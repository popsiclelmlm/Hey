...
| Persistent app settings | Present for core VPN, DNS, DNS hosts, IPv6, mux/XUDP/fragment, sniffing, log, real-ping concurrency, delete confirmation, immediate scan startup, and routing strategy; selected values are applied at connection start, batch test time, Scanner startup, or destructive actions, including VPN interface DNS, generated Xray `dns.hosts`, IPv6 routing, outbound Happy Eyeballs, mux settings, outbound fragment settings, delay-test concurrency, scan auto-launch, and deletion confirmation. |
