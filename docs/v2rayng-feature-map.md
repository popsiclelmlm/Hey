...
| Persistent app settings | Present for core VPN, DNS, DNS hosts, IPv6, mux/XUDP/fragment, sniffing, log, real-ping concurrency, and routing strategy; selected values are applied at connection start or batch test time, including VPN interface DNS, generated Xray `dns.hosts`, IPv6 routing, outbound Happy Eyeballs, mux settings, outbound fragment settings, and delay-test concurrency. |
